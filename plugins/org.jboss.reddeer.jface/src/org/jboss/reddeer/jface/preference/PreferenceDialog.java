...
public abstract class PreferenceDialog {
	...
	private void handlePreferencesOK(){
		org.jboss.reddeer.swt.api.Shell changeShell = null;
		String changeShellText = null;
		try{
			changeShell = new DefaultShell();
			changeShellText = changeShell.getText();
		if(changeShellText.equals(PROGRESS_SHELL_TITLE)){
			new WaitWhile(new ShellWithTextIsAvailable(PROGRESS_SHELL_TITLE));
			return;
		}
		} catch (RedDeerException ex){
			if(changeShell == null || (Boolean)ObjectUtil.invokeMethod(changeShell.getSWTWidget(), "isDisposed")){
				return;
			}
			throw ex;
		}
		...
		for(PreferencePage p: openedPreferecePages){
			if(changeShellText != null && changeShellText.equals(p.getPageChangedShellName())){
				p.handlePageChange();
				usedPage = p;
				break;
			}
